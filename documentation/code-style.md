...
Do not use `TypedDict`, in either functional or class syntax. `TypedDict` definitions make dictionary contracts excessively rigid, duplicate runtime validation, and make compatible evolution harder. Use ordinary `dict` annotations, and document meaningful fields through focused docstrings and behavior tests. Plain internal dictionaries do not need validation factories. Validate at trust boundaries, such as external API input, persistence decoding, configuration, and graph records, or when a compatibility or resource-bound invariant requires it. Where validation is required, runtime behavior remains authoritative.
