...
| Resource and failure behavior | `tests/test_resolvers.py`, `tests/test_claim_projection.py` | Claim count, evidence bytes, complete output, diagnostics, working memory, graph rows, vector results, and deadlines are bounded. Graph/model/index unavailability, resolver exceptions, projection conflicts, and partial dependency failure fail soft without leaking raw records. |
...
The artifact also proves order-independent canonicalization, the ten-record/64-KiB package gates, visible partial failure with retained EVIDENCE, containment of full records to the package, no response accounting for Claim-only evidence, exact output accounting, and working-memory compliance. Its synthetic data did not select or fit policy thresholds. The first fresh run while other host verification work was active failed the orchestration gate at 109.0144 ms p95; the recorded idle rerun passed at 25.7659 ms p95. These are local engineering measurements, not a release SLO.
