...
The versioned [engineering regression corpus](../../eval/section9-temporal-conflict-v1.json) is run by [benchmark_temporal_conflict.py](../../scripts/benchmark_temporal_conflict.py). Its legacy `held_out` split is visible repository material, not independent release evidence. The report binds to governed source state and records each case duration plus aggregate turn lengths; there are no latency answer budgets or pass/fail timing gates.
