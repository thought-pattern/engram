...
| EGR-1404 | `date_time_v1` requires Gregorian ISO dates or aware RFC3339 timestamps, rejects offset ambiguity, allow-lists five target zones, reports locked tzdata identity, ignores locale/current time, and uses canonical seconds-precision output. `unit_conversion_v1` fixes dimensions, factors, and 16-digit Decimal output. |
| EGR-1405 | `version_v1` implements only SemVer 2.0.0 precedence and `identifier_v1` implements only UUID/slug grammars. Neither value enters an import, evaluator, file, shell, subprocess, or network operation. |
...
The corpus has four conformance and two held-out cases per plugin. The benchmark
adds a plugin-specific resource exhaustion probe, one code-shaped threat input,
one algebraic/domain property, and 500 seeded matched-prefix fuzz cases per plugin
(3,500 total). Every corpus and property check passed; every resource probe
rejected with its declared error; no fuzz case caused an unexpected plugin failure
or oversized output.

| Plugin | Conformance | Held-out | Determinism | Fuzz failure | Observed p95 |
| --- | ---: | ---: | ---: | ---: | ---: |
| `arithmetic_v1` | 100% | 100% | 100% | 0% | 1.0485 ms |
| `boolean_v1` | 100% | 100% | 100% | 0% | 0.5753 ms |
| `set_v1` | 100% | 100% | 100% | 0% | 0.6264 ms |
| `date_time_v1` | 100% | 100% | 100% | 0% | 8.9738 ms |
| `unit_conversion_v1` | 100% | 100% | 100% | 0% | 0.9886 ms |
| `version_v1` | 100% | 100% | 100% | 0% | 0.4030 ms |
| `identifier_v1` | 100% | 100% | 100% | 0% | 0.3599 ms |
...
The review found and remediated two Section 14 defects before the final run:
...
- Focused utility suite: **42 passed**.
- Complete repository suite: **1,608 passed** in 102.46 seconds.
...
- The host's installed Black 26.5.1 package remains unusable because even its
  import/version probe does not complete; it is not misreported as a passing gate.
...
The benchmark, each per-plugin artifact, and the MCP evidence bind to governed
source SHA-256
