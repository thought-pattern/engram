...
class TemplateContext:
    ...
    # Substitution maps
    person_subs: dict[str, str] = field(default_factory=dict)
    person2_subs: dict[str, str] = field(default_factory=dict)
    gender_subs: dict[str, str] = field(default_factory=dict)
    ...
    # Redirect callback (set by processor)
    redirect_fn: object = None

    # Learn callback (set by processor)
    learn_fn: object = None
...
class TemplateProcessor:
    ...
    TRANSFORM_PATTERN = re.compile(r'\{(upper|lower|capitalize|formal|sentence|person|person2|gender|normalize|denormalize|explode|first|rest|uniq|wordcount):([^{}]*)\}')
    ...
    def _process_dict_template(self, template: dict, context: TemplateContext) -> str:
    ...
    def _process_random(self, choices: list, context: TemplateContext) -> str:
    ...
    def _process_condition(self, condition: dict, context: TemplateContext) -> str:
    ...
    def _process_sequence(self, sequence: list, context: TemplateContext) -> str:
    ...
    def _process_redirect(self, pattern: str, context: TemplateContext) -> str:
    ...
    def _process_think(self, items: list, context: TemplateContext) -> None:
        """Process think elements (silent, no output)."""
        for item in items:
            self.process(item, context)

    def _process_set(self, set_data: dict, context: TemplateContext) -> None:
    ...
    def _process_learn(self, learn_data: dict, context: TemplateContext) -> None:
        ...
        if context.learn_fn:
            ...
            if "pattern" in learn_data:
                resolved["pattern"] = self._substitute_variables(
                    learn_data["pattern"], context
                ).upper()
            if "template" in learn_data:
                # Resolve variables inside template
                resolved["template"] = self._resolve_template_vars(
                    learn_data["template"], context
                )
            if "that" in learn_data:
                resolved["that"] = self._substitute_variables(
                    learn_data["that"], context
                )
            if "topic" in learn_data:
                resolved["topic"] = self._substitute_variables(
                    learn_data["topic"], context
                )
            resolved["tier"] = learn_data.get("tier", "DYNAMIC")

            context.learn_fn(resolved)

    def _resolve_template_vars(self, template, context: TemplateContext):
    ...
    def _process_graph_query(self, query_data: dict, context: TemplateContext) -> str:
    ...
    def _process_graph_write(self, write_data: dict, context: TemplateContext) -> str:
    ...
    def _process_graph_delete(self, delete_data: dict, context: TemplateContext) -> str:
    ...
    def _process_triple_add(self, triple_data: dict, context: TemplateContext) -> str:
    ...
    def _process_triple_query(self, triple_data: dict, context: TemplateContext) -> str:
        ...
        result = context.graph_fn(query, params)
        if result and result.success and result.single:
            return str(result.single.get("result", ""))
        return ""
    ...
    def _apply_transforms(self, text: str, context: TemplateContext) -> str:
