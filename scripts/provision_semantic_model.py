...
def main() -> int:
    ...
    if destination.exists() and any(destination.iterdir()):
        ...
        print(json_dumps({**manifest, "manifest_path": manifest_path.as_posix(), "reused": True}, sort_keys=True))
        return 0
    ...
    with TemporaryDirectory(prefix=f".{destination.name}-", dir=destination.parent) as temporary_name:
        ...
        staged_manifest.write_text(json_dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        ...
        try:
            ...
            raise
    print(json_dumps({**manifest, "manifest_path": manifest_path.as_posix(), "reused": False}, sort_keys=True))
    return 0
