...
from engram.resolution import ResolutionBudget, ResolutionOutcome
...
def artifact(statement_id: str = "stmt-artifact") -> CachedResponseArtifact:
    ...
    return CachedResponseArtifact(
        ...
        superseded_by="",
        provenance=ArtifactProvenance("tapestry:test", "regulator", NOW_TEXT),
...
def test_feedback_contract_round_trip_is_deterministic_and_strict() -> None:
    ...
    assert FeedbackObservation.from_json(value.to_json()) == value
    assert value.relationship_key.from_json(value.relationship_key.to_json()) == value.relationship_key
    assert value.statement_key.from_json(value.statement_key.to_json()) == value.statement_key
    malformed = value.to_dict()
    malformed["schema_version"] = 2
    with pytest.raises(InvalidRequestError, match="schema_version"):
        FeedbackObservation.from_dict(malformed)
...
def test_feedback_receipts_apply_once_and_conflicting_retries_fail() -> None:
    ...
    replay = apply(store, "feedback-1", replace(value, observed_at="2026-08-15T12:00:01Z"))
    ...
    with pytest.raises(ConflictError, match="different observation"):
        apply(store, "feedback-1", replace(value, outcome=FeedbackOutcome.REJECTED_QUALITY))
...
def test_feedback_accepts_the_declared_thousand_observation_batch() -> None:
    ...
    values = tuple(replace(value, reference_id=f"resolution-{index}") for index in range(1_000))

    candidate = store.prepare("feedback-batch", values)

    receipt_result = cast(Any, candidate.receipt.to_dict()["result"])
...
def test_lifecycle_execution_status_does_not_change_feedback_retry_identity() -> None:
    ...
    assert first.replayed is False
    assert replay.replayed is True
    assert cast(Any, replay.receipt.to_dict()["result"])["lifecycle_status"] == "conflicted"
...
def test_feedback_partitions_isolate_scope_query_generation_and_policy() -> None:
    ...
    values = (
        ...
        replace(observation("d", FeedbackOutcome.REJECTED_CONTEXT), policy_fingerprint="b" * 64),
    )
...
def test_history_uses_sample_floor_priors_and_deterministic_aging() -> None:
    ...
    for index in range(4):
        apply(store, f"accepted-{index}", replace(accepted, reference_id=f"resolution-{index}"))
    ...
    apply(store, "accepted-4", replace(accepted, reference_id="resolution-4"))
    ...
    assert available.from_json(available.to_json()) == available


def test_feedback_capacity_and_bucket_retention_are_bounded_and_inspectable() -> None:
    policy = FeedbackPolicy(max_buckets_per_record=2, max_statement_records=2, max_relationship_records=2)
    store = FeedbackStore(FeedbackState(policy=policy))
...
def test_negative_store_has_fixed_ttl_capacity_exact_isolation_and_invalidation() -> None:
    ...
    changed_plan = replace(third, resolver_plan_fingerprint=canonical_fingerprint("plan-b"))
...
def test_negative_contract_requires_available_epoch_and_round_trips() -> None:
    key = negative_key()
    assert NegativeResolutionKey.from_json(key.to_json()) == key
    with pytest.raises(InvalidRequestError, match="available knowledge epoch"):
        replace(key, knowledge_epoch_available=False)
...
def test_non_exact_plans_do_not_cache_misses_that_can_hide_new_knowledge() -> None:
    ...
    core = EngramCore(engine, clock=lambda: NOW)
    budget = ResolutionBudget(total_time_ms=10_000, resolver_time_ms=10_000)
...
def test_negative_hit_honors_zero_diagnostic_budget() -> None:
    ...
    hit = core.resolve_request(
        ...
        budget=ResolutionBudget(max_diagnostic_bytes=0),
    )
...
def test_feedback_checkpoint_failure_retries_cached_resolution_without_double_credit(tmp_path, monkeypatch) -> None:
    ...
    core = EngramCore.open(store_path=store_path)
...
def test_feedback_checkpoint_marks_divergent_recovery_degraded(tmp_path, monkeypatch) -> None:
    ...
    assert core.engram.feedback_store.snapshot().statement_records == ()
    assert persistence.load_feedback_state(store_path).signature() == divergent_state.signature()
